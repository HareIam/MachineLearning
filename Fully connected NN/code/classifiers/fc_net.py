...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        layer1_relu,cache_relu = relu_forward(layer1)
        scores , cache2 = affine_forward(layer1_relu,W2,b2)
        ...
        derv1 = derv2 * (layer1 > 0)
        grads['W2'] = np.matmul(np.transpose(layer1_relu),derv3) + self.reg * W2
        grads['b2'] = np.sum(derv3,axis = 0)
        
       
        grads['W1'] = np.matmul(np.transpose(X),derv1) + self.reg * W1
        ...
        grads['W2'] = dw2
        grads['b2'] = db2
        dx_relu = relu_backward(dx2, layer1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            layer,cache = affine_relu_forward(layers[-1],W,b)
            if(self.use_dropout):
        ...
        scores,cache = affine_forward(layers[-1],W,b)
        caches.append(cache)
        if(self.use_dropout):
        ...
        #dw +=  self.reg * self.params["W"+str(len(caches))]
        grads['W'+str(len(caches))] = dw
        grads['b'+str(len(caches))] = db
