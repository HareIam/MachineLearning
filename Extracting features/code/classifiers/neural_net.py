...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # array of shape (N, C).                                               #         
        ########################################################################
        y0 = np.matmul(X,W1) + b1
        y1 = np.maximum(y0, 0, y0)
        scores = np.matmul(y1,W2) + b2
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
        ...
        ########################################################################
        y_exp = np.exp(scores - np.max(scores,axis=1,keepdims = True))
        softmax_prob = y_exp/(np.sum(y_exp,axis=1,keepdims = True))
        all_Examples = range(N)
        loss= -1*np.sum(np.log(softmax_prob[all_Examples,y]))/N
        loss+= 0.5 * reg * np.sum(np.square(W1)) + 0.5 * reg * np.sum(np.square(W2))
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
        ...
        #since we assume the fourth layer derivative is 1 we will start with third layer
        softmax_prob[all_Examples,y] -= 1
        derv3 = softmax_prob/(N-X.shape[1])
        derv2 = np.matmul(derv3,np.transpose(W2))
        derv1 = derv2 * (y0 > 0)
        grads['W2'] = np.matmul(np.transpose(y1),derv3) + reg * W2
        grads['b2'] = np.sum(derv3,axis = 0)
        
       
        grads['W1'] = np.matmul(np.transpose(X),derv1) + reg * W1
        grads['b1'] = np.sum(derv1,axis = 0)
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
    ...
    def predict(self, X):
        ...
        ########################################################################
        y0 = np.matmul(X,self.params['W1']) + self.params['b1']
        y1 = np.maximum(y0, 0, y0)
        scores = np.matmul(y1,self.params['W2']) + self.params['b2']
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    learning_rates = [13e-4]
    regularization_strengths = [13e-3]
    batch_size = [512]
    for learn in learning_rates:
        for reg in regularization_strengths:
            for batch in batch_size:
                ...
                net = TwoLayerNet(X_train.shape[1], 150, 10)
                stats = net.train(X_train, y_train, X_val, y_val,
                                  num_iters=1500, batch_size=batch,
                                  learning_rate=learn, learning_rate_decay=0.95,
                                  reg=reg, verbose=False)
                max_val_acc = np.max(stats["val_acc_history"])
                print("learn: ",learn," reg: ",reg," batch: ",batch," hidden: ",hidden," max_val ",max_val_acc)
                if(max_val_acc > best_val):
                    best_val = max_val_acc
                    best_net = net
                    #results[(learn,reg,batch,hidden)] = stats
